...
int send_file(int sockfd, struct sockaddr_in *client_addr, socklen_t client_addr_len, char *file_name) {
    ...
    do{
          ...
          //  printf("adding the size to the segments stack done\n");
           ...
           // printf("||| ------------ |||\n%s\n||| ------------ |||\n", buffer);
            ...
            if(flag_eof == 1){
               ...
               // flag_all_received = 1;
               window_size = (i > 0) ? i-1 : window_size;
                break;
            }
        }
        //printf("Sending buffer %s\n", buffer);
        ...
        for (int i = 0; i < loop_max && flag_all_received == 0 && flag_duplicated_ack == 0; i++)
        {
            ...
            tv.tv_sec = 0;
            tv.tv_usec = 1.9*estimate_timeout(acks->RTT); //DEFAULT_TIMEOUT; //estimate_timeout(acks->RTT); // Set the timeout based on the last received RTT.
